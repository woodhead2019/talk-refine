...
| 组件 | 模型 | 内存占用 | CPU（空闲时） | 说明 |
|------|------|----------|---------------|------|
| **ASR** | SenseVoice-Small | ~3 GB | ~0% | 加载后常驻 |
| **LLM** | Qwen3.5:2b (Ollama) | ~5 GB | ~0% | 智能内存管理（见下方） |
| **合计** | — | **~8 GB** | — | 同时运行时的峰值 |

### 智能内存管理

LLM 模型的内存由 TalkRefine 自动管理，无需手动干预：
