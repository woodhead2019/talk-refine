...
- 🗣️ **语音 → 文字 → 精炼文字**，一步到位
- 🔒 **完全离线**，数据不出本机
- ⚡ **极速**，10 秒语音不到 1 秒识别，LLM 润色 ~3-4 秒
- 🌍 **多语言**，中文、英文、日文、韩文等
...
| **LLM (llama.cpp)** | Qwen3.5-4B-Q4_K_M | ~1.6 GB（增量） | 进程内加载，~3-4s 推理，无额外服务开销 |
...
| **语音识别 + llama.cpp 润色**（推荐） | llama.cpp | ~4.6 GB | 轻量离线润色，无需额外服务 |
...
| 场景 | 行为 | LLM 内存 |
|------|------|----------|
| 正常使用 | 模型常驻内存，响应 ~3-4 秒 | ~4 GB (llama.cpp) |
